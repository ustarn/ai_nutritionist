...
app.get("/api/weight/summary", authenticateToken, async (req, res) => {
  try {
    ...
    if (targetWeight && startWeight && currentWeight) {
      ...
      progressPct =
        totalDelta !== 0
          ? Math.min(1, Math.max(0, doneDelta / totalDelta))
          : null;
    }
...
请严格按照以下JSON格式返回，不要包含任何多余文字：
{
      ...
      messages: [
        {
          role: "system",
          content:
            "你是一名专业的营养师，擅长根据用户的身体状况制定个性化饮食计划。",
        },
        {
          role: "user",
          content: prompt,
  ...
  const elapsed = Date.now() - startTime;
  console.log(
    `✅ AI 推荐 API 请求完成, 耗时: ${elapsed}ms (${(elapsed / 1000).toFixed(
      1
    )}秒), 状态码: ${response.status}`
  );
...
app.post("/api/ai/meal-recommendation", authenticateToken, async (req, res) => {
  try {
    ...
    const recommendation = await generateMealRecommendationWithAI(
      profile,
      goal
    );
